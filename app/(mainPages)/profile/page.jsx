...
const images = [ProfileImg1.src, ProfileImg2.src, ProfileImg3.src, ProfileImg4.src, ProfileImg5.src, ProfileImg6.src]
...
=======
			...
			<div className="flex justify-between mt-3">
				<p className="font-normal cursor-pointer"><span className="font-semibold">40</span> Diary</p>
...
>>>>>>> ceebe94669dddc44128251821a07c48b709fb5c1
		...
		<div className="flex bg-white w-80 mt-[40px]">
			<button 
				className={`center w-40 px-8 py-4 font-medium tracking-widest text-sm ${myDiaries ? "text-[#68BB59] border-b-2 border-b-[#68BB59] " : "text-[#808080]"}`}
				onClick={() => setMyDiaries(!myDiaries)}
			>
				MY DIARIES
			</button>
			<button 
				className={`center w-40 px-9 py-4 font-medium tracking-widest text-sm ${!myDiaries ? "text-[#68BB59] border-b-2 border-b-[#68BB59] " : "text-[#808080]"}`}
				onClick={() => setMyDiaries(!myDiaries)}
			>
				SAVED
			</button>
		...
		<div className="grid grid-rows-2 grid-cols-3 gap-3 mt-3">
			{
				images.map((image, index) => {
					return (
						<Image key={index} src={image} alt="" width={206} height={242} className="cursor-pointer" />
					)
				})
			}
		</div>
